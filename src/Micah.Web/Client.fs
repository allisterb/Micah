...
module Client =
    ...
    let run() =        
        ...
        CUI.EchoDoc <| Doc.Concat [
            Bs.btnSuccess "hello" (fun _ _ -> initMic(main');initSpeech();quick_utter "greet" |> push |> Micah.Web.Main.update)
            Html.text "     "
            Bs.btnPrimary "help" (fun _ _ -> initMic(main');initSpeech();quick_utter "help" |> push |> Micah.Web.Main.update)
        ]
